...
config = yaml.load(open("config/musicldm_audiostock10k/musicldm_audiostock10k.yaml", 'r'), Loader=yaml.FullLoader)
...
class z_matching(pl.LightningModule):
    def __init__(self, batch_size, model, learning_rate=0.0001, dataset=None):
        super().__init__()
        # self.save_hyperparameters()
        self.vector = nn.Parameter(torch.randn(batch_size,8,256,16))  # Trainable vector
    ...
    def separate(self):
        ...
        if unconditional_guidance_scale != 1.0:
            unconditional_conditioning = (
                self.frozen_network.cond_stage_model.get_unconditional_condition(b)
            )
        ...
        with torch.no_grad():
            ...
            z = 1.0 / self.frozen_network.scale_factor * img
    ...
    def forward(self):
        ...
        # Sum along axis 1
        summed_waveform = waveform.sum(axis=1)
    ...
    def on_train_epoch_end(self, unused=None):
        ...
        if self.current_epoch % 10 == 0:
            ...
            log_dict [f"New mixture"] = wandb.Audio(new_mix[0], sample_rate=16000, caption=f"new mixture")
